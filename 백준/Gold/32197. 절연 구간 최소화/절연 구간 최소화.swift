...
while !q.isEmpty{
    ...
    if minCost[curr.node][curr.type] < curr.cost { continue }
    
    for next in map[curr.node]{
        let newCost = curr.cost + (next.type != curr.type ? 1:0)
        if minCost[next.node][next.type] > newCost{
            minCost[next.node][next.type] = newCost
            q.insert((next.node, newCost, next.type))
        }
    }
}
