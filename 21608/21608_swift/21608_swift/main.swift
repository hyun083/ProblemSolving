...
func cntLovers(x:Int, y:Int, lovers:[Int]) -> (Int,Int){
    var heart = 0
    var blank = 0
    let dx = [-1,1,0,0]
    let dy = [0,0,-1,1]
...
for _ in 0..<n*n{
    ...
    for i in 0..<n{
        for k in 0..<n{
            if map[i][k] == -1{
                let info = cntLovers(x: i, y: k, lovers: lovers[num])
                let love = info.0
                let blank = info.1
...
for i in 0..<n{
    for k in 0..<n{
        let info = cntLovers(x: i, y: k, lovers: lovers[map[i][k]])
        ans += happy[info.0]
    }
}
print(ans)
